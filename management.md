...
### Group Task 17 — Profile open/download button in navbar + mobile sidebar — DONE 2026-07-13
Source: user request (2026-07-13). "add a profile open/download btn in navbar, when in mobile screen be in sidebar."

**What exists already:** every service/product page's sidebar (`.sidebar-card.sidebar-download`) and the site footer already have a "تحميل البروفايل" (download profile) button linking to `assets/profile.pdf`. Neither the desktop navbar nor the mobile offcanvas menu had one — that was the actual gap filled here.

**⚠️ Pre-existing gap found while checking this:** `assets/profile.pdf` does not exist anywhere in the repo (`find . -iname "*profile*"` returns nothing but the HTML that references it). Every existing profile-download button on the site (sidebar widget on ~34+ pages, footer on every page) has therefore always been broken — this predates today's work and is not something this task introduced. The new nav/offcanvas buttons point to the same conventional path (`assets/profile.pdf`) so they start working the moment the file is uploaded, consistent with the rest of the site — did not fabricate a placeholder PDF.

**Real discovery made while investigating the navbar structure:** `components/header-home.html` — despite its name suggesting it's the homepage header — is **never actually used by any live page**. `components-loader.js` only loads it when `<body data-header="home">` is set, and a full-site grep found zero pages with that attribute (the default is `'inner'`). `index.html` doesn't use the shared header system at all — the comment in `embed-components.js` ("skip index — it has inline header") confirms it, and it has its own fully separate, hand-baked `<header>` block. So the button had to go in **3 places**, not 2: `components/header-inner.html` (covers ~60 pages live), `index.html`'s own inline header (edited directly, since no component covers it), and `components/header-home.html` (edited too, for consistency/future-proofing even though currently dead code — flagged in case someone activates it later).

**What was built:**
1. Desktop navbar: a compact teal pill button (download icon + "البروفايل", `.rs-header-profile-btn`, `download` attribute matching the site's existing convention) added to `components/header-inner.html`, `index.html`'s inline header, and `components/header-home.html` — all `d-none d-lg-flex` so it only shows at desktop widths and never collides with the hamburger-collapsed mobile nav.
...
### Group Task 19 — Break up monotonous side-image row runs — DONE 2026-07-14
Source: user request — "wide side side ok but wide side side side side side side not cool, randomize the layout... dont edit content just this layout."

**Problem:** the Group 14.1 side-image conversion (image + text in a `col-lg-6`/`col-lg-6` row, alternating left/right) looked fine for 1–2 rows in a row, but on pages with 3+ consecutive steps it produced a visually monotonous unbroken column of side-by-side rows.

**Method — measured before touching anything:** grepped every page using `side-img` for the line positions of `row align-items-center` (side rows) and both `section-heading` (H2) and `<h3` (step headings, whether inside a row or standalone) to reconstruct the *true* visual sequence per section — a raw count of `row` divs between two H2s was not enough, since some pages already had a standalone (imageless) H3 breaking up the run. Only sections with **3+ genuinely consecutive** side rows (no break at all) were touched; sections with 2 consecutive were left alone, matching the user's own "wide side side ok" example.

**6 pages / 7 sections had a real 3+ run and were fixed** by converting selected rows from the 2-column side-by-side wrapper back to a full-width stacked layout (`<h3>` → full-width `<picture>/<img class="section-img">` → `<p>`, the same pattern already used for each page's own intro image) — varying which rows became "wide" per page so the site doesn't read as one repeated template:
- `bitumen-waterproofing.html` (4 rows): wide, side, side, wide
- `polyurethane-waterproofing.html` (3 rows, preceded by an existing text-only step): side, wide, side
- `epoxy-lining-potable-water-tanks.html` (3 rows): side, wide, side
- `epoxy-flooring-food-processing-facilities.html` (3 rows): side, wide, side
- `terrazzo-flooring.html` ("كيف تُنفّذ" section, 4 rows): wide, side, wide, side
- `microcement-flooring.html` ("كيف تُنفّذ" section, 4 rows): side, wide, side, wide

**2 sections that matched the naive "3 rows between headings" grep but were already fine on closer inspection were correctly left untouched:** `cementitious-waterproofing.html`'s steps section (already has a standalone text-only step splitting the run into 2+2) and `microcement-flooring.html`'s "تطبيقات" applications section (already has 3 standalone text-only application headings splitting the run into 1+2).

**Nothing else changed:** no text, alt text, image files, headings, SEO, or JSON-LD touched on any of the 6 pages — purely the row/column wrapper markup for the specific rows converted, reusing the site's existing `.section-img` class (no new CSS).

**Verified:** re-read every edited section to confirm balanced markup; a div/picture open-close tag count sanity check on all 6 files matched exactly, except `bitumen-waterproofing.html` which showed a 1-tag imbalance — confirmed via `git show HEAD` that the exact same imbalance already existed in the file before today's edit (pre-existing, same class of issue flagged during the Group 14.1 rollout), not introduced by this change.

### Group Task 20 — Modernize the legacy detail-page sidebar — DONE 2026-07-14
Source: `newtasks.md`, the item right after the "from here" marker — "fix side bar content and make it look a bit modern." (The other post-marker item, "profile btn open/download in all navbars + working sidebar btn," was already fully delivered by Group Task 17 — verified still in place, no further action needed.)

**Problem found:** the site actually had two parallel sidebar systems. Hub pages (epoxy-flooring-coating.html, concrete-repair-injection.html, structural-strengthening.html, waterproofing-insulation.html, our-services.html, certificates.html) and the 2 newest services (terrazzo/microcement) use a clean custom system built this session — `.sidebar-card` / `.sidebar-nav-list` / `.sidebar-download`, teal brand color, rounded cards. But all 36 other pages using `.rs-sidebar-wrapper` were still on the **original stock-theme classes** (`.sidebar-widget`, `.widget-categories-two`, `.widget-download`) — checked the CSS and confirmed these render with square corners (`border-radius:2px`), a boxy `::before` accent bar, and — the real bug — an **orange hover accent** (`--rs-theme-orange`) that doesn't match the site's teal brand color used everywhere else. Genuinely dated and inconsistent, not just a style preference.

**Scope — every page using the old sidebar, split into 3 shapes:**
- **27 core service detail pages** (all of Groups 2, 3, 5, 6, 7 + the epoxy/thermal/injection/support detail pages): sidebar has a WhatsApp CTA + sibling-category nav list + company-profile download + phone/email contact card.
- **6 product pages** (`adhesive-materials.html`, `cementitious-waterproofing-products.html`, `construction-materials.html`, `insulation-drain.html`, `insulation-rolls.html`, `product-rain.html`): sidebar has only a category list, no CTA/download/contact — plus a commented-out dead "Filter by price" shop-theme widget (leftover from the original template, never active).
- **3 blog pages** (`concrete-crack-repair-techniques.html`, `excavation-shoring-support.html`, `roof-waterproofing-thermal-insulation.html`): sidebar has a site-nav-links list (no title) + a "مدونات اخرى" (other blogs) card-style widget.
- **3 pages already modern, correctly left untouched:** `terrazzo-flooring.html`, `microcement-flooring.html`, `waterproofing-thermal-insulation-system.html` — built this session already on `.sidebar-card`/`.sidebar-nav-list`.

**What changed — wrapper classes and list markup only, zero text/link/content changes:**
- Category/nav lists: `.sidebar-widget.widget-categories-two` (or `.widget-categories`) → `.sidebar-card`, its `<ul>` → `.sidebar-nav-list`, unwrapped the product pages' extra `.sidebar-widget-content > .list` nesting layer. Every link, href, and label preserved exactly; only added the `ri-arrow-left-line` arrow icon + `active`-on-self marking to the 6 product pages' list (they had neither), matching the pattern already used everywhere else on the site.
- Company-profile download widget (27 core pages): unwrapped the old `<ul><li><a><div class="left">...</div></a></li></ul>` nesting into the flat `<a><img><span><i></a>` structure already used on every hub sidebar — same link, same icon, same "تحميل البروفايل" text.
- Contact card (27 core pages): wrapper's ad-hoc inline `style="background:#f8fafb;border:1px solid #e8edf2;..."` replaced with the shared `.sidebar-card` class (same visual result, less duplicated inline CSS); phone/email links untouched.
- 6 product pages: also deleted the dead commented-out "Filter by price" block while touching this section (confirmed dead — a leftover shop-theme filter never wired to anything, consistent with dead-code removals made earlier in the rebuild).
- `.cta-whatsapp` (top WhatsApp button) was left as its own class everywhere — checked its CSS and it's visually identical to the hub's `.sidebar-cta` (same teal gradient, same rounded style), so no conversion was needed there.

**Method:** did the 27 core pages with one whitespace-tolerant PowerShell regex pass (3 substitutions × 27 files, verified each pattern matched exactly once per file before writing — no blind overwrite), the 6 product pages with a second regex pass driven by a shared link table (per-file `active` class computed from filename), and the 3 blog pages by hand via `Edit` since their structure had small per-file variation not worth risking a shared regex over.

**Verified:** re-read a sample from each of the 3 groups post-edit to confirm clean markup; confirmed zero remaining `.sidebar-widget.widget-categories-two` / `.widget-download` / `.widget-categories` references anywhere in the repo; div-count balance check across a sample from each group matched exactly (the one pre-existing `bitumen-waterproofing.html` imbalance, already documented in Group 19, shifted by the same fixed amount and is unrelated to this edit).

### Group Task 21 — index.html nav gap + offcanvas mobile menu fix — DONE 2026-07-14
Source: user request — "the last 2 services are not in index navbar in services fix" + "in site-offcanvas i want 2 things: 1 is not appearing in all sub pages, 2 fix the content in and make it look a bit modern."

**Part A — index.html missing services (quick, verified bug):** `index.html` has its own separate hand-baked header (confirmed back in Group 17 — it doesn't use `components/header-inner.html` at all), and its submenu still only listed the original 4 services, missing `terrazzo-flooring.html` and `microcement-flooring.html` added in Group 16. Checked `components/header-inner.html` and `components/header-home.html` first — both already had all 6 links correctly (Group 16 did update those) — so this was specifically an index.html-only gap. Added the 2 missing `<li>` entries to match.

**Part B — offcanvas investigation.** Read `assets/js/components-loader.js` and `components/offcanvas.html` and found two real, separate bugs, not just one:

1. **`index.html`'s `<div id="site-offcanvas">` was completely empty** (`<div id="site-offcanvas"></div>`) — unlike every other page (~64 pages), which has the full offcanvas panel baked directly into its raw HTML as a fallback, with `components-loader.js` only doing a *live refresh* on top via synchronous XHR. On index.html there was no baked fallback at all — if the live JS injection is ever slow, blocked, or fails (e.g. opening the file directly via `file://` instead of a server, which browsers block synchronous cross-origin XHR for), the homepage's mobile menu button would do nothing, while every sub-page would still show its baked (if stale) content. This explains "not appearing" specifically not matching what sub-pages do.
2. **The offcanvas panel's actual navigation menu was genuinely empty everywhere, including live.** `.mobile-menu > .rs-offcanvas-menu` contained just `<nav></nav>` with nothing in it. Traced this by reading `assets/js/plugins/meanmenu.min.js` directly rather than assuming: `main.js` calls `$("#mobile-menu").meanmenu({ meanMenuContainer: ".mobile-menu", ... })`, and meanmenu.js *does* `.prepend()` a `<div class="mean-bar"><nav class="mean-nav"></nav></div>` into whatever `.mean-container` is, filled with a clone of the desktop nav — so in theory this should have worked. But the resulting `.mean-nav` menu inherits the theme's generic default styling only (no teal branding, not matching the rest of the redesigned site), and depends entirely on meanmenu.js executing successfully after the offcanvas is in the DOM — a fragile, invisible-until-JS-runs dependency, which was very likely also the source of the "not appearing" complaint. Confirmed via `grep` there is zero purpose-built CSS anywhere in `main.css` for a clean offcanvas menu list — the theme never really finished this component.

**Fix:** replaced the dependency on meanmenu entirely for the offcanvas panel with a real, static, modern nav list — reliable regardless of JS/CORS conditions and fully teal-branded to match the rest of the site:
- Added `.offcanvas-menu-block` / `.offcanvas-nav-list` / `.offcanvas-submenu` CSS to `main.css` (teal `#3fada2` hover/active, chevron rotates on expand, light `#eee` dividers — same visual language as `.sidebar-nav-list` used everywhere else this session).
- Built the actual menu: الرئيسية / من نحن / خدماتنا (collapsible, all 6 services incl. تيرازو + مايكروسمنت) / منتجاتنا (collapsible, all 6 products) / الشهادات / بعض مشاريعنا / المدونة / تواصل معنا — using Bootstrap's `collapse` (already loaded, same mechanism as the FAQ accordions) for the 2 submenus, no new JS.
- Applied this to `components/offcanvas.html` (the live-loaded source of truth) and, via a whitespace-tolerant regex pass verified to match exactly once per file, to all **64 pages'** own baked copies of the old empty block (since `embed-components.js`'s re-bake script is confirmed broken and can't be used to propagate this automatically).
- **Also found and fixed a second staleness gap while comparing baked vs. live content:** the Group 17 profile-download block that's in `components/offcanvas.html` had never been baked into any page's own copy — inserted it into all 64 pages in the same pass, right after the new nav menu.
- Finally baked the complete, now-correct offcanvas block (nav menu + profile-download + contact + social, byte-identical to every other page) directly into `index.html`'s previously-empty div, closing the original "not appearing" gap at its root.

**Verified:** confirmed zero pages still reference the old empty `<nav></nav>` placeholder or are missing the profile-download block (`grep` returned none); div open/close tag counts matched exactly on a sample from each page type (index.html, a service page, a hub page, certificates.html) after the edit.

### Group Task 22 — Sticky header hamburger + offcanvas email visibility — DONE 2026-07-15
Source: user report with 2 screenshots — "in the navbar in sub pages the Offcanvas btn dont appear, make it appear in all sub pages, probably u just need to change color, and in Offcanvas add the email it dont appear."

**Bug 1 — hamburger icon invisible after scrolling.** The screenshots confirmed Group 21's new offcanvas menu itself works correctly (nav list, collapsible submenus, download button, contact info all rendering) — the actual complaint was the header's sidebar-toggle (hamburger) button that *opens* the offcanvas. Traced the sticky-header behavior in `main.js`: on scroll past 250px, JS adds class `rs-sticky` to `#header-sticky` (`.rs-header-area.rs-header-one`), which a custom rule (`.rs-sticky { background: rgb(0,8,7) !important; }`, near the bottom of `main.css`) turns near-black. But the *matching* rule meant to flip all the header's text/icons to white on that dark background was targeting `.rs-header-inner.sticky` — a class that is **never actually added by any script** (the real class is `rs-sticky`, on a different, outer element). This dead selector meant the color-flip rule silently never fired: after scrolling, the header background went black but the hamburger icon (and technically all nav links) kept their original dark color — a dark-gray icon on a near-black background is effectively invisible, exactly matching "btn dont appear."
- Fixed the selector to match what JS actually applies: `.rs-header-one.rs-sticky .rs-header-inner a { color: #fff; }` and a new explicit `.rs-header-one.rs-sticky .bar-icon svg path { fill: #fff; }` rule (the hamburger's SVG has a hardcoded `fill="#616161"` attribute, which — unlike text — does *not* respond to a CSS `color` change, so it needed its own rule).
- Also added `.rs-header-hamburger { flex-shrink: 0; }` as a defensive measure so the icon can't get squeezed to near-zero width in the flex row on narrower desktop widths.

**Bug 2 — offcanvas email hidden behind the floating WhatsApp button.** The site has a global fixed WhatsApp action button (`.whatsapp-float`, `position:fixed; bottom:30px; left:30px; z-index:1000`). In RTL the offcanvas panel slides in and sits at that same physical left edge, and `.offcanvas-area` only has `z-index:999` — one below the WhatsApp button. Since the panel's contact section (location → phone → **email**) sits near the bottom of the panel, right where the floating WhatsApp button lives, the button was rendering *on top of* the email row, hiding it — while location and phone (higher up in the panel) stayed clear. Confirmed by the user's second screenshot showing the green WhatsApp circle sitting right at the cutoff point.
- Fixed by raising `.offcanvas-area.info-open` to `z-index: 1001` (only while open) — the panel now always renders above the floating WhatsApp button, so its own contact info (including email) is never obscured. The WhatsApp button's normal z-index is untouched for every other state.
...
| 2026-07-14 | 19 | Broke up monotonous 3+ consecutive side-image row runs on 6 pages (7 sections) by converting select rows to full-width, varied per page — no content changed | ✅ Done |
...
### 2026-07-13 — Group Task 17: profile open/download button (navbar + mobile sidebar)
- User request: "add a profile open/download btn in navbar, when in mobile screen be in sidebar."
- Checked what already exists first: every service/product page's sidebar and the site footer already have a "تحميل البروفايل" button → `assets/profile.pdf`. Neither the desktop navbar nor the mobile offcanvas had one — confirmed that's the actual gap, not a duplicate of existing work.
- **Found while checking:** `assets/profile.pdf` doesn't exist anywhere in the repo (`find . -iname "*profile*"` returns only HTML references). Every profile-download button already on the site — sidebar widgets, footer — has therefore always 404'd. Pre-existing, not introduced today. Wired the new buttons to the same path so they'll just work once the PDF is uploaded, rather than inventing a placeholder file.
- **Found while wiring the navbar:** `components/header-home.html` is dead code. `components-loader.js` only loads it when `<body data-header="home">` is set; a full-site grep for that attribute returned zero matches. `index.html` doesn't use the shared header system at all — it has its own hand-baked `<header>` (confirmed by `embed-components.js`'s own comment: "skip index — it has inline header"). So the button needed 3 places, not the 2 originally planned: `components/header-inner.html` (the one that's actually live on ~60 pages), `index.html`'s inline header (edited directly), and `header-home.html` (edited too, for consistency/in case it's ever wired up — flagged as dead code either way).
...
### 2026-07-14 — Group Task 19: break up monotonous side-image row runs
- User flagged a layout problem: pages built during the Group 14.1 side-image rollout looked fine with 1–2 consecutive side-by-side image/text rows ("wide side side ok") but turned repetitive with 3+ in an unbroken run ("wide side side side side side side not cool"). Asked to randomize the layout (mix in full-width rows) without touching any content.
- Before editing anything, mapped every `side-img`-using page's true row sequence (not just a raw row count between headings) by cross-referencing `row align-items-center` positions against both H2 and H3 line positions, since several pages already had standalone (imageless) H3 steps that break up a run visually even though a naive count would flag them.
- Found and fixed 7 sections across 6 pages with a genuine unbroken 3+ run: `bitumen-waterproofing.html` (4→wide/side/side/wide), `polyurethane-waterproofing.html` (3→side/wide/side), `epoxy-lining-potable-water-tanks.html` (3→side/wide/side), `epoxy-flooring-food-processing-facilities.html` (3→side/wide/side), `terrazzo-flooring.html`'s execution section (4→wide/side/wide/side), `microcement-flooring.html`'s execution section (4→side/wide/side/wide). Varied the wide/side placement per page so the fix doesn't read as one templated pattern repeated everywhere.
- Correctly identified and left untouched 2 sections that superficially matched the "3 rows" grep but already had a natural break: `cementitious-waterproofing.html` (a standalone text-only step already splits its run 2+2) and `microcement-flooring.html`'s applications section (3 standalone text-only headings already split its run 1+2).
- Converted rows reuse the site's existing `.section-img` class (the same one used for every page's own intro image) for the "wide" treatment — no new CSS, no text/alt/image changes, purely wrapper markup.
- Verified: re-read every edited section for balanced markup; div/picture tag-count check on all 6 files matched, except `bitumen-waterproofing.html` which has a 1-tag imbalance confirmed via `git show HEAD` to already exist before this edit (pre-existing, unrelated).

### 2026-07-14 — Group Task 20: modernize the legacy detail-page sidebar
- Source: `newtasks.md`, the "fix side bar content and make it look a bit modern" item after the "from here" marker. Checked the other post-marker item ("profile btn") first — already fully delivered by Group Task 17, nothing left to do there.
- Investigated before touching anything: found two parallel sidebar systems on the site. The 7 hub pages plus terrazzo/microcement already use a clean custom system built this session (`.sidebar-card`/`.sidebar-nav-list`/`.sidebar-download`, teal brand color). The other 36 pages using `.rs-sidebar-wrapper` were still on the original stock-theme `.sidebar-widget` classes — checked `main.css` and confirmed these render with square 2px corners and, notably, an **orange hover accent** (`--rs-theme-orange`) that doesn't match the site's teal brand color used everywhere else. A real inconsistency, not just a style opinion.
- Split the 36 pages into 3 shapes by content: 27 core service detail pages (CTA + category nav + download + contact card), 6 product pages (category list only, plus a dead commented-out "Filter by price" shop-widget), 3 blog pages (site-nav list + "مدونات اخرى" other-posts widget).
- Converted all 36: swapped `.sidebar-widget`/`.widget-categories-two`/`.widget-download`/`.widget-categories` wrapper classes for `.sidebar-card`/`.sidebar-nav-list`/`.sidebar-download`, unwrapped the product pages' extra nesting layer, unwrapped the download widget's `<ul><li><div class="left">` structure into the flat pattern already used on hub pages, and replaced the contact card's ad-hoc inline background style with the shared `.sidebar-card` class. Every link, href, label, and phone/email untouched — verified `.cta-whatsapp` (kept as-is) is CSS-identical to the hub's `.sidebar-cta`, so no need to touch that button. Deleted the dead "Filter by price" widget on the 6 product pages while there (confirmed genuinely dead, never wired up).
- Method: whitespace-tolerant PowerShell regex for the 27 core pages (3 patterns × 27 files, each verified to match exactly once before writing) and the 6 product pages (link table + per-file `active` class), direct `Edit` calls for the 3 blog pages since their structure had small per-file variation.
- Verified: confirmed zero remaining old-class references site-wide (`grep -l` came back empty), spot-read a sample from each of the 3 shapes post-edit, and re-ran the div-balance sanity check — matched everywhere except the already-documented pre-existing `bitumen-waterproofing.html` imbalance from Group 19 (unrelated to this edit).

### 2026-07-14 — Group Task 21: index.html nav gap + offcanvas mobile menu fix
- User reported 2 things: (1) the 2 newest services missing from index.html's navbar, (2) the mobile offcanvas menu "not appearing in all sub pages" and needing modernized content.
- Part A: confirmed `components/header-inner.html` and `components/header-home.html` already had all 6 services (Group 16 covered those) — only `index.html`'s own separate hand-baked header (it doesn't use the shared header components at all, established back in Group 17) was missing تيرازو/مايكروسمنت. Added the 2 links.
- Part B: read `components-loader.js` and `components/offcanvas.html` before touching anything, and found two distinct bugs behind the single complaint:
  1. `index.html`'s `<div id="site-offcanvas">` was completely empty in raw HTML, unlike every other page which has the whole panel baked as a fallback with `components-loader.js` only live-refreshing it via synchronous XHR on top. No fallback on index.html meant any JS/CORS hiccup (e.g. opening via `file://`) left the homepage's mobile menu doing nothing, while sub-pages still rendered their (stale but present) baked copy.
  2. The offcanvas panel's actual nav menu (`.mobile-menu > .rs-offcanvas-menu > nav`) was a genuinely empty placeholder everywhere, live included. Read `meanmenu.min.js` directly to confirm rather than guess: `main.js` points meanmenu at `.mobile-menu` as its container, and meanmenu does clone the desktop nav in there via `.mean-nav` — but that only shows up if JS actually finishes running, and it renders with the theme's stock styling, not the site's teal branding. Confirmed via `grep` there's zero custom CSS anywhere for this component — never really finished by the original theme.
- Fix: stopped depending on meanmenu for the offcanvas menu and built a real static one instead. Added `.offcanvas-menu-block`/`.offcanvas-nav-list`/`.offcanvas-submenu` CSS to `main.css` (teal accents, chevron rotation, same visual language as `.sidebar-nav-list`). Built the actual menu — all main nav items plus both submenus (6 services incl. the 2 new ones, 6 products) using Bootstrap `collapse` (already loaded for the FAQ accordions, no new JS). Applied to `components/offcanvas.html` and, via a whitespace-tolerant regex verified to match exactly once per file, to all 64 pages' baked copies.
- Along the way, found a second staleness gap: the Group 17 profile-download block in `components/offcanvas.html` had never been baked into any page — inserted it into all 64 pages in the same pass.
- Finally baked the complete corrected offcanvas (nav + download + contact + social) into `index.html`'s previously-empty div, matching every other page byte-for-byte.
- Verified: `grep` confirmed zero pages still have the old empty `<nav></nav>` placeholder or a missing profile-download block; div balance checked clean on a sample across page types.

### 2026-07-15 — Group Task 22: sticky header hamburger + offcanvas email visibility
- User sent 2 screenshots: the (working, per Group 21) offcanvas menu open on a sub-page, plus a report that the header's hamburger button that opens it "dont appear" on sub-pages, and that the email link inside the offcanvas contact section "dont appear" either — with their own guess that it was "probably" a color issue.
- Hamburger bug: traced `main.js`'s scroll handler (line ~380) — past 250px scroll it adds class `rs-sticky` to `#header-sticky`, and a separate `!important` rule turns that background near-black. But the rule meant to flip text/icons to white on that dark background was scoped to `.rs-header-inner.sticky` — a class string that literally nothing in any script ever adds (the real class is `rs-sticky`, added to a different, outer element). Confirmed by checking the JS directly rather than trusting the existing CSS comment. Fixed the selector to the real one (`.rs-header-one.rs-sticky ...`) and added an explicit SVG-fill rule for the hamburger specifically, since its icon color comes from a hardcoded `fill="#616161"` attribute that a CSS `color:#fff` rule can't touch. Also added `flex-shrink:0` to the hamburger's container as a defensive measure against the icon getting squeezed at narrower desktop widths.
- Email bug: found the site's fixed floating WhatsApp button (`.whatsapp-float`, bottom:30px, left:30px, z-index:1000) sits at the same physical corner the offcanvas panel occupies once opened (RTL slide-in from the left), and the panel itself was `z-index:999` — one below. Since the panel's contact list (location → phone → email) reads top-to-bottom with email last, near the bottom of the panel exactly where the floating button sits, the WhatsApp button was rendering on top of the email row and hiding it, while location/phone (higher up) stayed visible — matching the screenshot exactly. Fixed by raising `.offcanvas-area.info-open` to `z-index:1001`, only while the panel is open.
- Before diagnosing further, ruled out markup corruption from earlier bulk edits by directly checking `blogs.html`'s live hamburger HTML — confirmed intact, so the bug was purely CSS.
- Both fixes are additive, CSS-only, in `main.css` — no HTML touched, applies site-wide automatically via the shared stylesheet, no per-page propagation needed.
...
| 2026-07-14 | Group Task 19 — break up monotonous side-image row runs | bitumen-waterproofing.html, polyurethane-waterproofing.html, epoxy-lining-potable-water-tanks.html, epoxy-flooring-food-processing-facilities.html, terrazzo-flooring.html, microcement-flooring.html | `949f8dc` (commit before this task) | Only the row/column wrapper markup for specific already-existing steps changed (side layout → full-width `.section-img` layout); no text, alt, image files, or other sections touched. `git checkout 949f8dc -- <file>` reverts any single page to its all-side layout |
